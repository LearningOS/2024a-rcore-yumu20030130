...
pub fn sys_mutex_lock(mutex_id: usize) -> isize {
    ...
    drop(process_inner);
    drop(process);
    if current_process().inner_exclusive_access().check_deadlock(0) {
        -0xdead
    } else {
        mutex.lock();
        current_process().inner_exclusive_access().mutex_available[mutex_id] -= 1;
        current_process().inner_exclusive_access().mutex_allocated[tid][mutex_id] += 1;
...
pub fn sys_semaphore_down(sem_id: usize) -> isize {
    ...
    process_inner.sem_need[tid][sem_id] += 1;
    drop(process_inner);
    if process.inner_exclusive_access().check_deadlock(1) {
        -0xdead
    }
    else {
        sem.down();
        process.inner_exclusive_access().sem_available[sem_id] -= 1;
        process.inner_exclusive_access().sem_allocated[tid][sem_id] += 1;
        process.inner_exclusive_access().sem_need[tid][sem_id] -= 1;
        0
    }
}
